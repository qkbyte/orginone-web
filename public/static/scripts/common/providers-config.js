...
flowableModule.factory("NotPermittedInterceptor", [
  "$q",
  "$window",
  function($q, $window) {
    return {
...
flowableModule.config([
  "$httpProvider",
  function($httpProvider) {
    if (!$httpProvider.defaults.headers.get) {
      $httpProvider.defaults.headers.get = {};
    }
    $httpProvider.defaults.headers.common["Authorization"] = "Basic c3dvcmQ6c3dvcmRfc2VjcmV0";
    $httpProvider.defaults.headers.common["blade-auth"] = getQueryString(
      "token"
    );
