<template>
  ...
  >
      ...
      >
        ...
        <el-table-column label="操作" align="center">
          <template slot-scope="{ row }">
            <el-button
              icon="el-icon-delete"
              @click.native.stop="deleteListener(row)"
            ></el-button>
          </template>
        </el-table-column>
      </el-table>
    ...
    <div style="text-align: right; padding: 10px">
      <el-button icon="el-icon-plus" type="primary" @click="addListener"
        >添加</el-button
      >
      <el-button icon="el-icon-close" @click="_listenerDrawer = false"
        >取消</el-button
      >
...
export default {
  ...
  methods: {
    init() {
      ...
      this.listeners =
        this.element.businessObject.extensionElements && this.element.businessObject.extensionElements.values
          .filter(
            (item) =>
              item.$type === this.descriptorPrefix + this.type + "Listener"
          )
          .map((item) => {
            let type;
    ...
    editListener(row) {
      ...
      this.listenerParams = this.listeners[index] && this.listeners[index] .params || [];
    },
    editListenerParams() {
    ...
    save() {
      ...
      this.listeners.forEach((item) => {
        const listener = this.modeler
          .get("moddle")
          .create(this.descriptorPrefix + this.type + "Listener");
      ...
      this.updateProperties({
        extensionElements:
          extensionElements.get("values") && extensionElements.get("values").length == 0
            ? undefined
            : extensionElements,
      });
      this._listenerDrawer = false;
      this.$emit("saveListener", this.listeners && this.listeners.length);
    },
  },
};
</script>
