<template>
  <DiyImport
    :tableName="tableName"
    :tableHead="tableHead"
    uploadUrl="/dev-api/asset-system/async/importJob"
...
export default {
  ...
  methods: {
    async loadData(params, callback) {
      params.tableName = '租户岗位表'
      const res = await getTenantImportHistory(params);
      callback(res);
    },

    async handleModel(callback) {
      const { data: res } = await getPostModel();
      let fileName = "岗位导入模板.xls";
    ...
    async handleImport(params, callback) {
      ...
      try{
        this.isImp = false
        const res = await importPosition(params);
        callback(res.data);
        this.isImp = true
      }catch{
        if(!this.isImp){
