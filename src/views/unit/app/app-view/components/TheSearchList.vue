<template>
  <div class="search-list" style="height:100%">
    ...
    <div v-show="selectType !== 2" class="head-layout">
      ...
      <div  class="number">{{checkList.length}}/{{getSeletctList.length}}/{{total}}</div>
    </div>
...
export default {
  ...
  computed:{
    getSeletctList(){
      return this.$parent.$parent.selectList
    }
  },
  created() {
    this.listOption.label = this.title;
        ...
        let handler = (event) => {
          ...
          if (scrollTop + windowHeight + 0.5 >= scrollHeight) {
            this.timer = setTimeout(()=>{
              ...
              this.$parent.$parent.isCurrent = true;
              if(this.selectType == 0){
                this.$parent.$parent.handleNode(this.searchId)
              }else{
                this.$parent.$parent.loadAllList(this.arrList)
              }
  ...
  methods: {
    handleCheckAllChange(val){ // 全选的变化函数
      this.checkAll = val
    ...
    getChoseTree(){ // 用来获取已选中的树
      ...
      for (let i = 0; i < this.arrList.length; i++) {
        if(this.arrList[i].roleId == this.curTab){
          if(this.selectType == 0){
            let arr = []
            if(this.arrList[i].userIds.length !== 0){
              this.arrList[i].userIds.forEach((el)=>{
                arr = [...arr, el.userId]
              })
              arr = arr.splice(',')
              this.$refs.tree.setCheckedKeys(arr)
              this.$parent.$parent.selectList = this.arrList[i].userIds
            }
            break;
          }else if(this.selectType == 1){
            let arr = []
            if(this.arrList[i].jobIds.length !== 0){
              this.arrList[i].jobIds.forEach((el)=>{
                arr = [...arr, el.jobId]
              })
              arr = arr.splice(',')
            ...
            let arr = []
            if(this.arrList[i].agencyIds.length !== 0){
              this.arrList[i].agencyIds.forEach((el)=>{
                arr = [...arr, el.agencyId]
              })
              arr = arr.splice(',')
              this.$refs.tree.setCheckedKeys(arr)
      ...
      this.$parent.$parent.tabSwitch = true
    },
    handleCheck(data) {
    ...
    handleCheckChange(data, checked, indeterminate) {
      ...
      for (let i = 0; i < arr.length; i++) {
        if(arr[i].roleId == this.curTab){
          if(this.selectType == 0){ // 按人员分配
            if(checked){
              let handle = false
            ...
            }else if(this.$parent.$parent.tabSwitch == true && checked == false){
          ...
          }else if(this.selectType == 1){ // 按岗位分配
            if(checked){
              let handle = false
