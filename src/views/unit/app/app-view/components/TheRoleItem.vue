<template>
  <div class="role-item">
    ...
    <div class="role-item__number">
      <div
        v-if="sendData.distributionPerson !== 0"
        class="role-item__number--yes "
      >
        <i class="el-icon-circle-check"></i>
        已分配 {{ sendData.distributionPerson }} 人
      </div>
      <div v-else class="role-item__number--no ">
        <i class="el-icon-remove-outline"></i>
        未分配人员
      </div>
      <div
        class="role-item__number--yes "
        v-if="sendData.distributionDept !== 0"
      >
        <i class="el-icon-circle-check"></i>
        已分配 {{ sendData.distributionDept }} 部门
      </div>
      <div v-else class="role-item__number--no">
        <i class="el-icon-remove-outline"></i>
        未分配部门
      ...
      <div
        class="role-item__number--yes"
        v-if="sendData.distributionJob !== 0"
      >
        <i class="el-icon-circle-check"></i>
        已分配 {{ sendData.distributionJob }} 岗位
      </div>
      <div v-else class="role-item__number--no">
        <i class="el-icon-remove-outline"></i>
        未分配岗位
