    ...
    <div class="app-info">
      <div class="diy-dialog-body">
        <div class="app-info_body">
          ...
          <TheAppMenu ref="menu" @handleUpdate="handleUpdate" :appId="menu[0].appId" :menuList="menuList" :roleNameList="role" :release="release"></TheAppMenu>
          <div class="title">菜单预览</div>
          <TheRoleMenu ref="role" :roles="roles" :tree="menuTree"></TheRoleMenu>
          <div class="title">应用组件</div>
          <TheAppRelease ref="appRelease" :appId="appId" :componentList="componentList"></TheAppRelease>
        </div>
      </div>
    </div>
  </div>
...
import TheRoleMenu from "./TheRoleMenu";
import TheAppRelease from "./TheAppRelease"
...
export default {
  ...
  methods: {
    handleUpdate(roleList, menuList) {
      ...
      this.$parent.menu[0].icon = menuList[0].icon
    },
    getDealMenu(arr){
