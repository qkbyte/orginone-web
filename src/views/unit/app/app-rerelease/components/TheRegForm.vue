...
export default {
  data() {
    var valiVersion = (rule, value, callback) => {
      if (value === "") {
        ...
        if (pattern.test(value)) {
          if (this.compareVersion(this.form.version, this.version) < 0) {
            callback(new Error("所输入的版本号不能小于上版本的版本号"));
          } else {
            callback();
          }
        } else {
    ...
    return {
      ...
      rules: {
        ...
        contact: [{ required: true, message: "请输入负责人联系方式", trigger: "blur" }],
        description: [
  ...
  watch: {
    form: {
      handler(newValue) {
        Bus.$emit("form", newValue);
      },
      deep: true,
    },
    inputImg: {
      handler(newValue) {
        Bus.$emit("inputImg", newValue);
      },
      deep: true,
    },
  },
