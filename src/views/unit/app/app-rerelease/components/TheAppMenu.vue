...
<script>
import { getTenanticonList, isDeleteRole, isAddRole } from "@api/market";
...
export default {
  ...
  watch: {
    ...
    menuTree: {
      handler(val) {
        this.$parent.$refs.role.tree[0].menuName = val[0].menuName;
        this.$parent.$refs.role.tree[0].menuUrl = val[0].menuUrl;
      },
      deep: true,
    },
  },
  ...
  mounted() {
    this.getIconList();
    Bus.$on("form", (msg) => {
      this.menuTree[0].menuName = msg.appName;
    });
    Bus.$on("inputImg", (msg) => {
      this.menuTree[0].icon = msg;
    });
  },
  methods: {
    ...
    getMenuTree(val) {
      ...
      for (let i = 0; i < val.length; i++) {
        ...
        data.roleNameList = val[i].roleNameList;
        if (val[i].children) {
          data.children = this.getChild(val[i].children);
        }
        tree = [...tree, data];
      }
      tree[0].menuUrl = val[0].menuUrl;
      tree[0].roleNameList = val[0].roleNameList;
      this.menuTree = tree;
      console.log("this.menu", this.menuTree);
    },
    getChild(val) {
      let obj = val;
      for (let i = 0; i < obj.length; i++) {
        if (typeof val[i].children == "undefined") {
          obj[i].children = [];
        } else {
          this.getChild(val[i].children);
        }
      }
      return obj;
    },
    ...
    async handleClose(tag) {
      ...
      await isDeleteRole(params).then((res) => {
        code = res.data.code;
      });
      if (code == 200) {
        let changeTree = function deepTraversal(node, key) {
          let nodes = [];
          if (node !== null) {
            if (node.roleNameList.includes(key)) {
              node.roleNameList.splice(node.roleNameList.indexOf(key), 1);
            }
            nodes.push[node];
            if (node.children) {
