    ...
    <DiyBoxLayout>
      <div class="layout-content">
        ...
        <div
          ...
          @mousedown="move"
        >
          <div class="layout-menu__head">设为首页</div>
          <TheSearchInput
            placeholder="输入模板"
          ...
          <div v-show="!searchShow" class="layout-menu-body__treeList">
            <el-collapse v-model="activeNames">
              <el-collapse-item title="系统模板" name="1">
                <div
                  class="tree"
                  @drag="drag"
                  ...
                  draggable="true"
                  unselectable="on"
                  :class="defaultList == item.id ? 'defaultClick' : ''"
                ...
                >
                  <div class="row">
                    <div
                      ...
                      :class="defaultList == item.id ? 'el-icon-star-on' : ''"
                    ></div>
                    <div>
                      {{ item.name }}
                    </div>
              ...
              <el-collapse-item title="自定义模板" name="2">
                <div
                  class="tree"
                  @drag="drag"
                  @dragend="dragend(item)"
          ...
          <!-- 旧版 -->
          <!-- <div class="layout-menu__head">
            <div class="layout-menu__save" @click="handleSave">保存</div>
            <div class="layout-menu__row">
              <div
                class="layout-menu__left"
                :class="listShow ? 'layout-menu__onClick' : ''"
                @click="listShow = true"
              >
                组件
              </div>
              <div
                class="layout-menu__right"
                :class="!listShow ? 'layout-menu__onClick' : ''"
                    ...
                    >
                      <div class="row">
                        <div
                          ...
                          @click="handleDefault(item, index)"
                          :class="
                            defaultList == item.id ? 'el-icon-star-on' : ''
                          "
                        ></div>
                        <div>
                          {{ item.name }}
...
export default {
    ...
    MenuStyle() {
      ...
      if (this.menuShow && this.menuBtnShow) {
        if (this.BtnDirection == "right") {
          menu = "right:0px;transition: all 0.5s ease;";
        } else {
          menu = "margin-left:0px;transition: all 0.5s ease;";
        }
  ...
  mounted() {
    ...
    // this.getDefaultList(); // 获取该用户默认设置的模板
    // this.loadlistData(); // 加载组件
    this.loadTemplateData(); // 加载模板
    // this.loadUserTemplate(); // 加载自定义模板
    // this.getUserUse()
  ...
  methods: {
    ...
    //   };
    //   getTemplateIdByUserIdInUse(params).then((res) => {
    //     this.userUse = res.data.data;
    ...
    //   });
    //   if (judge) {
    //     this.getLayoutList(this.customList);
    //   } else {
    ...
    loadUserTemplate(val, str) {
      ...
      getMarketappcomponenttemplateGetUserTemplate(params).then((res) => {
        ...
        //  else {
        //   // this.getUserUse();
        //   this.handleSystemShow();
        // }
      });
    },
    // handleSystemShow() {
    //   // 用于判断当自定义模板已存在默认时 系统模板的默认不显示
    //   this.customList.forEach((el) => {
    //     if (el.isDefault == 1) {
    //       this.systemShow = true; // 则系统模板默认不显示
    //       throw Error();
    //     } else {
          ...
          for (let i = 0; i < this.userIdList.length; i++) {
            ...
            records.forEach((el) => {
              if (this.userIdList[i] == el.appId && el.status == 1) {
                arrList.push(el);
              } else if (this.userIdList[i] !== el.appId && el.status == 1) {
                arr.push(el);
              }
            });
          }
    ...
    handleUserLink() {
      ...
      let params = {
        userId: this.userId,
        useStatus: 1, // -1为未使用 1为使用
        templateId: this.customList[this.customList.length - 1].id,
      };
      marketappusertemplateSubmit(params);
    },
    ...
    resizeEvent: function(i, newH, newW, newHPx, newWPx, item) {
      console.log("item", this.$refs.gridItem);
      let data = JSON.parse(JSON.stringify(item));
      switch (item.layoutType) {
        case "height":
          this.$refs.gridItem.forEach((el) => {
            if (data.i == el.i) {
              el.maxH = data.h;
              el.minH = data.h;
            }
