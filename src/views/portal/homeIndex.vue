            ...
            >
              ...
              <div style="height: 100%">
                <div
                  class="transparent"
                  ref="transparent"
                  v-show="transparentShow || !menuListShow"
                ></div>
                <SandBox
                  v-if="item.type == 1"
            ...
            <el-collapse v-model="activeNames">
              ...
              <el-collapse-item title="自定义模板" name="2">
                <div
                  ...
                  draggable="true"
                  unselectable="on"
                  @click="addprotalCustom(item, index)"
                  :class="
                    defaultOnClick
                      ? customIndex === index
                        ? 'defaultClick'
                ...
                >
                  <div
                    class="row"
                    style="padding:5px 0;justify-content:space-between"
                  >
                    <div style="display:flex;flex-direction:row">
                      <div
                        ...
                        :class="defaultList == item.id ? 'el-icon-star-on' : ''"
                      ></div>
                      <div class="templateText" :title="item.name">
                        {{ item.name }}
                      </div>
                    </div>
                    <div
                      @click="handleDeleteCustomList(item)"
          ...
          <div v-show="searchShow" class="layout-menu-body__treeList">
            ...
            >
              <el-popover placement="right" width="400" trigger="hover">
                ...
                <div slot="reference" class="el-icon-setting listContain_item">
                  {{ item.name }}
                </div>
              </el-popover>
            </div> -->
...
import TheSaveJobDialog from "./components/SaveJobDialog";
...
export default {
  ...
  methods: {
    ...
    async setHomePage() {
      if (JSON.stringify(this.templateData) !== "[]") {
        await this.handleDefault(this.templateData);
      }
    ...
    async loadAppData() {
      try {
        ...
        appComponentGroupByUser(params)
          .then((res) => {
            this.newAppList = res.data.data;
            for (let key in this.newAppList) {
              let c = this.newAppList[key].filter(
                (item) =>
                  this.userIdList.indexOf(item.appId) !== -1 && item.status == 1
              );
              if (c.length == 0) {
                delete this.newAppList[key];
              } else {
                this.newAppList[key] = c;
              }
            }
          })
          .catch((err) => {
    ...
    addprotalCustom(data, index) {
      // 点击自定义模板事件
      this.customIndex = index;
      this.systemIndex = false;
      this.templateData = data;
      var config = JSON.parse(data.config);
      this.layout = config;
    },
