<template>
  <DiyButton>
    <template v-slot:opt>
      <div class="diy-button" v-if="row.noticeReleaseStatus == 0" @click="handleActive(index, row)">发布</div>
      <div class="diy-button" v-else @click="handleInactive(index, row)">下架</div>
      <div class="diy-button" @click="handleEdit(0, row)">编辑</div>
      <div class="diy-button" @click="handleDelete(0, row)">删除</div>
    </template>
  </DiyButton>
</template>

<script>
import DiyButton from "@components/DiyButton/index";
import { mapGetters } from "vuex";
import { getAppInfo, unsubscribeApp } from "@api/market";

export default {
  data() {
    return {};
  },
  computed: {
    ...mapGetters("tenant", ["curTenantCode"]),
  },
  components: { DiyButton },
  created() {},
  props: ["row", "index"],
  methods: {
    /**编辑角色 */
    handleEdit(index, row) {
      this.$emit("handleEdit", index, row);
    },
    /**删除角色 */
    handleDelete(index, row) {
      this.$emit("handleDelete", index, row);
    },
    handleActive(index, row) {
      this.$emit("handleActive", index, row);
    },
    handleInactive(index, row) {
      this.$emit("handleActive", index, row);
    },
    /*
      配置角色
    */
    handleAuth(index, row) {
      this.$emit("handleAuth", index, row);
    },
  },
};
</script>

<style lang="scss" scoped></style>
