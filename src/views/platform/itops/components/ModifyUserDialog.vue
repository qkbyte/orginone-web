<template>
  ...
  >
    ...
    <div class="diy-dialog-body">
      <el-select
        v-model="value"
        :placeholder="tableData[index].tenant_name"
        :style="{ width: '100%' }"
      >
        <el-option
...
import { getmultitenant, modifyuser } from "@api/itops";
...
export default {
  ...
  methods: {
    ...
    loadtenantlist() {
      let params = {
        phone_number: this.dialogShow.sendData.phone_number,
      };
      getmultitenant(params).then((res) => {
        let _this = this;
    ...
    //   }
      ...
      let data = {
       tenantcode:_this.tableData[_this.index].tenant_code,
       phonenumber:this.dialogShow.sendData.phone_number,
       itopsdetailid:_this.dialogShow.sendData.id,
      };
      console.log('data',data);
      modifyuser(data).then(
        (res) => {
          this.loading = false;
