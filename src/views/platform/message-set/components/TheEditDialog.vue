    ...
    >
      <el-form class="demo-ruleForm" :model="ruleForm" :rules="rules" label-width="100px" ref="ruleForm">
        <el-form-item label="接收人" prop="receiver">
          <sy-remote-select class="width" v-model="ruleForm.receiver" :apiPromise="apiPromise" :result="ruleForm.personList" disabled />
        </el-form-item>
...
import { getAllUserInfo, updateMessage } from "@api/message-set";
...
export default {
  ...
  watch:{
    dialogShow:{
      handler(val){
        this.ruleForm = { ...val.sendData, receiver: val.sendData.personList.map(j => j.userId),
          personList: val.sendData.personList.map(i => { return { ...i, userName: i.name } }) }
      },
      deep:true,
      immediate:true
    }
  ...
  methods: {
    ...
    handleClick() {
      this.$refs['ruleForm'].validate().then(() => {
        this.loading = true
        updateMessage(this.ruleForm).then(() => {
