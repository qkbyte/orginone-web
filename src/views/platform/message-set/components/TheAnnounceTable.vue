  ...
  <div style="height:100%;">
    ...
    >
      ...
      <template v-slot:noticeUnitIds="scope">
        <el-tag v-if="scope.row.noticeUnitIds[0].type == 1">信息上报</el-tag>
        <el-tag v-else-if="scope.row.noticeUnitIds[0].type == 2" type="success"
          >日常通知</el-tag
        >
        <el-tag v-else-if="scope.row.noticeUnitIds[0].type == 3" type="danger"
          >安全生产</el-tag
        >
    ...
    <template v-for="item in dialogShow">
      <TheAnnounceEditDialog
        v-if="item.key === 'edit' && item.value"
        :key="item.key"
        :dialogShow="item"
        @closeEdit="closeEdit"
      ></TheAnnounceEditDialog>
      <TheAnnounceAddDialog
...
<script>
import TheAnnounceButton from "./TheAnnounceButton";
...
import TheAnnounceEditDialog from "./TheAnnounceEditDialog";
import TheAnnounceAddDialog from "./TheAnnounceAddDialog";
export default {
  data() {
    let _this = this;
    return {
