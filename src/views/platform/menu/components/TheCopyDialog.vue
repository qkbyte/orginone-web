...
export default {
  ...
  created() {
    ...
    for (let i in this.form) {
      this.form[i] = this.dialogShow.sendData[0][i];
    }
    this.loadMenuTree();
  },
  computed: {
  ...
  methods: {
    loadMenuTree() {
      getMenuTree().then((res) => {
        ...
        let index = _.findIndex(this.menuTree, function(e) {
          return e.id == _this.form.parentId;
        });
        if (index === -1) {
          this.msg = "请选择上级菜单";
        } else {
          this.msg = this.menuTree[index].name;
        }
      });
    },
    handleClose() {
